...
public sealed class BingXWebSocketClientTests
{
    ...
    public void ParseUpdates_ParsesLiveFlatArrayPayload()
    {
        const string payload = """
            {"code":0,"dataType":"BTC-USDT@kline_5m","s":"BTC-USDT","data":[{"c":"78005.4","o":"78090.9","h":"78141.4","l":"77955.6","v":"164.5371","T":1787750100000}]}
            """;

        var update = Assert.Single(BingXWebSocketClient.ParseUpdates(payload));

        Assert.Equal(TimeFrame.FiveMinutes, update.TimeFrame);
        Assert.Equal(DateTimeOffset.FromUnixTimeMilliseconds(1787750100000), update.Candle.OpenTime);
        Assert.Equal(update.Candle.OpenTime.AddMinutes(5), update.Candle.CloseTime);
    ...
    {
        const string payload = """
            {"dataType":"BTC-USDT@kline_5m","data":{"K":{"t":1787750100000,"T":1787750399999,"o":"100","h":"110","l":"90","c":"105","v":"12"}}}
            """;
        ...
        Assert.Equal(TimeFrame.FiveMinutes, update.TimeFrame);
        Assert.Equal(DateTimeOffset.FromUnixTimeMilliseconds(1787750100000), update.Candle.OpenTime);
        Assert.Equal(DateTimeOffset.FromUnixTimeMilliseconds(1787750399999), update.Candle.CloseTime);
        Assert.Equal(105m, update.Candle.Close);
        Assert.False(update.Candle.IsClosed);
    }

    [Fact]
    public void ParseUpdates_ParsesFifteenMinutePayload()
    {
        const string payload = """
    ...
    public void ClosedCandleTracker_EmitsLatestSnapshotOnceOnRollover()
    {
        ...
        var next = CreateUpdate(firstOpen.AddMinutes(5), 106m, 1m);
    ...
    public void ClosedCandleTracker_DiscardsAStaleSnapshotAfterRestReconciliation()
    {
        ...
        tracker.DiscardThrough("BTC-USDT", TimeFrame.FiveMinutes, firstOpen.AddMinutes(5));
    ...
    private static MarketUpdate CreateUpdate(DateTimeOffset openTime, decimal close, decimal volume) =>
            new("BTC-USDT", TimeFrame.FiveMinutes,
            new Candle(openTime, openTime.AddMinutes(5), close - 1m, close + 1m, close - 2m, close, volume, false));
}
